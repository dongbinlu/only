                                ...
                                <#if orderSimpleReport??>
                                    <#if orderSimpleReport.orderCompleteCount!=0>
                                        <span class="glyphicon glyphicon-ok"></span>&nbsp;<span
                                            class="simpleOrderReport">${orderSimpleReport.orderCompleteCount!""}</span>个交易完成.
                                    </#if>
                                    <#if orderSimpleReport.orderCancelCount!=0>
                                        <span class="glyphicon glyphicon-remove"></span>&nbsp;<span
                                            class="simpleOrderReport">${orderSimpleReport.orderCancelCount!""}</span>个取消.
                                    </#if>
                                    <#if orderSimpleReport.orderWaitPayCount!=0>
                                        <span class="glyphicon glyphicon-time"></span>&nbsp;<span
                                            class="simpleOrderReport">${orderSimpleReport.orderWaitPayCount!""}</span>个等待付款.
                                    </#if>
                                <#else>
                                    无任何订单数据！
                                </#if>
                                        ...
                                        <tr id="delete${item.id!""}">
                                            <td>&nbsp;
                                                <div style="width:50px;height: 50px;border: 0px solid;float: left;margin-left: 20px;">
                                                    <a href="${basepath}/product/${item.productID!""}.html"
                                                       target="_blank" title="${item.productName!""}">
                                                        <img style="width: 100%;height: 100%;border: 0px;" alt=""
                                            ...
                                            <#if item_index == 0>
                                                <td style="text-align: center;border-left:1px solid #ddd;vertical-align: middle;"
                                                    ...
                                                    <#if item.paystatus?? && item.paystatus=="y">
                                                        ...
                                                        <#elseif item.status?? && item.status =="send">
                                                            <a target="_blank" href="http://www.alipay.com"
                                                               class="btn btn-primary btn-sm">确认收货</a>
                                                        <#elseif item.status?? && item.status =="sign">
                                                            <!-- 已签收 -->
                                                        <#else>
                                                            <!-- 交易完成 -->
                                                    ...
                                                    <#else>
                                                        <a target="_blank"
                                                           href="${basepath}/order/toPay?id=${item.id!""}"
                                                           class="btn btn-danger btn-sm">付款</a>
                                                    </#if>
                                                    <br>
                                                    <#if item.status?? && (item.status =="cancel" || item.status=="file")>
                                                        ...
                                                        <#else>
                                                            <a target="_blank"
                                                               href="${basepath}/order/rate?orderid=${item.id!""}"
                                                               class="btn btn-danger btn-sm">我来评价</a><br>
                                                        </#if>
                                                    </#if>
                                                    ...
                                                    <#if item.status?? && (item.status !="send" &&item.status !="sign")>
                                                        <a href="javascript:void(0)"
                                                           onclick="return deleteOrder(${item.id!""})">删除订单</a>
                                                    </#if>
                                                    <#--   href="${basepath}/order/deletes?ids=${item.id!""}"-->
                                                    <br>
                                                    <#if item.status?? && (item.status =="send" || item.status=="sign")>
                                                        <a target="_blank"
                                                           href="http://www.kuaidi100.com/chaxun?com=${item.expressCompanyName!""}&nu=${item.expressNo!""}">物流详情</a>
                                                    </#if>
                                                </td>
                                            </#if>
                                        </tr>
