        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <#if e.rateOrderdetailList??>
                    ...
                    <form action="${basepath}/order/doRate?orderid=${e.id!""}" id="form" method="post"
                          onsubmit="return checkRate();">
                        <#list e.rateOrderdetailList as item>
                            <div class="row">
                                <div class="col-xs-12">
                                    <div class="media">

                                        <a class="pull-left" href="${basepath}/product/${item.productID!""}"
                                           target="_blank" title="${item.productName!""}">
                                            <img class="media-object err-product"
                                                 style="width: 100px;height: 100px;border: 0px;" alt=""
                                                 src="${systemSetting().imageRootPath}/${item.picture!""}"
                                                 onerror="nofind()"/>
                                        </a>
                                        <div class="media-body">
                                            <h5 class="media-heading">对【${item.productName!""}】进行评价：</h5>
                                            <textarea class="form-control" name="product_${item.productID!""}" rows="4"
                                                      cols="80"></textarea>
                                        </div>
                                    </div>
                                </div>
