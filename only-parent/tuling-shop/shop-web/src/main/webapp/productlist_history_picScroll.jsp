...
<style type="text/css">
    ...
    .picScroll-top .hd .prev, .picScroll-top .hd .next {
        ...
        overflow: hidden;
        cursor: pointer;
        background: url("<%=request.getContextPath() %>/resource/js/superSlide/demo/images/icoUp.gif") no-repeat;
    }

    .picScroll-top .hd .next {
        background: url("<%=request.getContextPath() %>/resource/js/superSlide/demo/images/icoDown.gif") no-repeat;
    }
            ...
            <s:iterator value="#application.suijiProducts" status="i" var="row">
                <li>
                    <div class="pic"><a href="http://www.SuperSlide2.com" target="_blank">
                        <img border="0"
                             src="<%=SystemManager.getInstance().getSystemSetting().getImageRootPath()%><s:property escape="false" value="picture" />"/></a>
                    </div>
                    <div class="title">
...
<script type="text/javascript">
    jQuery(".picScroll-top").slide({
        titCell: ".hd ul", mainCell: ".bd ul", autoPage: true, effect: "top", autoPlay: true,
        scroll: 5, vis: 5
    });
</script>
