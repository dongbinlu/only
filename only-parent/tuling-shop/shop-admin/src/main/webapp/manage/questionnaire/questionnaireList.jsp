    ...
    <table class="table table-bordered">
        ...
        <tr>
            <td colspan="16">
                ...
                <s:submit method="deletes" onclick="return deleteSelect('确定删除选择的记录?');" value="删除"
                          cssClass="btn btn-danger"/>
                    <%-- 							<s:submit method="updateInBlackList" onclick="return updateInBlackList();" value="加入黑名单" cssClass="btn btn-warning" /> --%>
                <s:submit method="setStatusY" value="显示" onclick="return deleteSelect('确定让选择的记录审核通过?这样选择的记录将会出现在门户上。');"
                          cssClass="btn btn-warning"/>
                <s:submit method="setStatusN" value="不显示" cssClass="btn btn-warning"
        ...
        <s:iterator value="pager.list">
            <tr>
                ...
                <td><s:a href="questionnaire!toEdit.action?e.id=%{id}">编辑</s:a>|
                    <a href="<%=SystemManager.getInstance().getSystemSetting().getWww() %>/question/show.html?id=<s:property value="id" />"
                       target="_blank">查看</a>
                </td>
            </tr>
        </s:iterator>
