...
<div style="text-align: center;border-bottom: 1px solid #ccc;padding: 5px;">
    ...
    <s:else>
        <a class="btn btn-warning"
           href="<%=SystemManager.getInstance().getSystemSetting().getWww() %>/question/show.html?id=<s:property value="e.id" />"
           target="_blank">查看</a>
        <s:submit method="update" onclick="return onSubmit();" value="保存问卷" cssClass="btn btn-primary"/>

        <s:a href="questionnaire!toEdit.action?e.id=%{e.id}&e.addQuestionnaireItem=true&e.editQuestionnaireItem=false"
                ...
                <td style="text-align: left;">
                    ...
                    <s:if test="e.editQuestionnaireItem">
                        <s:iterator value="e.questionnaireItem.optionList" status="i" var="item">
                            选项：<input type="text" name="e.questionnaireItem.optionArr" id="option" width="400px" maxLength="100" value="<s:property
                                value="item"/>"/><br>
                        </s:iterator>
                    </s:if>

                    选项：<s:textfield name="e.questionnaireItem.optionArr" id="option" placeholder="请输入选项内容" width="400px"
                                    maxLength="100"/><br>
                    选项：<s:textfield name="e.questionnaireItem.optionArr" id="option" placeholder="请输入选项内容" width="400px"
                                    maxLength="100"/><br>
                    选项：<s:textfield name="e.questionnaireItem.optionArr" id="option" placeholder="请输入选项内容" width="400px"
            ...
            <tr>
                <td colspan="2" style="text-align: center;">
                    <s:if test="e.questionnaireItem.id=='' or e.questionnaireItem.id==null">
                        <s:submit method="insertQuestionItem" action="questionnaire" onclick="return onSubmit(this);"
                                  value="新增题目" cssClass="btn btn-warning"/>
                    </s:if>
                    <s:else>
                        <s:submit method="insertQuestionItem" action="questionnaire" onclick="return onSubmit(this);"
                                  value="修改题目" cssClass="btn btn-warning"/>
                    </s:else>
