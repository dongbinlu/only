        ...
        <table class="table table-bordered">
                ...
                <td colspan="2" style="text-align: center;">
                    <#if e.id??>
                        ...
                        <#if e.status??&&e.status=="y">
                            <a action="news" id="btnDown" href="down?id=${e.id}" class="btn btn-warning"
                               onclick="return confirm(\" 确定不显示此文章吗?\");">
                            <i class="icon-arrow-down icon-white"></i> 不显示</a>
                        <#else>
                            <a action="news" id="btnUp" href="up?id=${e.id}" class="btn btn-warning"
                        ...
                        <#elseif e.type??&&e.type=="help">
                            <a class="btn btn-info" target="_blank"
                               href="${systemSetting().www}/help/${e.code!""}.html">
                                <i class="icon-eye-open icon-white"></i> 查看</a>
                        </#if>
            ...
            <#if e.type??&&e.type=="help">
                <tr>
                    ...
                    <td>
                        <select onchange="catalogChange(this)" name="catalogID" id="catalogSelect"
                                data-rule="类别:required;catalogSelect;">
                            <option></option>
                            <#list catalogsArticle as item>
                                <option pid="0" <#if e.catalogID?? && item.id==e.catalogID>selected="selected" </#if>
                                        value="${item.id!""}"><font color='red'>${item.name!""}</font></option>
                            </#list>
                        </select>
                    </td>
                </tr>
                <tr>
                    ...
                    <td style="text-align: left;"><input type="text" value="${e.code!""}" name="code"
                                                         data-rule="文章编码:required;code;length[1~25];remote[unique]"
                                                         id="code"/><br>
            ...
            <tr>
                ...
                <td style="text-align: left;"><input type="text" value="${e.title!""}" name="title" style="width: 80%;"
                                                     id="title"
