            ...
            <tr>
                ...
                <td>
                    ...
                    <select id="paystatus" name="paystatus" class="search-query input-medium">
                        <#list map?keys as key>
                            <option value="${key}"
                                    <#if e.paystatus?? && e.paystatus==key>selected="selected" </#if>>${map[key]}</option>
                        </#list>
                    </select>
                </td>
                <td>时间范围</td>
                <td><input id="d4311" class="Wdate search-query input-small" style="width:80px" type="text"
                           name="startDate"
                           value="${e.startDate!""}"
                           onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'d4312\')||\'2020-10-01\'}'})"/>
                    ~
                    <input id="d4312" class="Wdate search-query input-small" style="width:80px" type="text"
                           name="endDate"
                           value="${e.endDate!""}"
                ...
                <td colspan="21">
                    ...
                    <select id="refundStatus" name="refundStatus" class="search-query input-medium">
                        <#list map?keys as key>
                            <option value="${key}"
                                    <#if e.refundStatus?? && e.refundStatus==key>selected="selected" </#if>>${map[key]}</option>
                        </#list>
                    </select></td>
            </tr>
            <tr>
        ...
        <table class="table table-bordered table-hover">
            ...
            <#list pager.list as item>
                <tr>
                    ...
                    <td><a target="_blank"
                           href="${basepath}/manage/account/show?account=${item.account!""}">${item.account!""}</a></td>
                    <#--<%-- 					<td>${item.rebate!""}</td> --%>-->
