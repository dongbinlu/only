    ...
    <form action="${basepath}/manage/order" method="post" theme="simple" name="form" id="form">
        ...
        <div id="buttons" style="text-align: center;border-bottom: 1px solid #ccc;padding: 5px;">
            ...
            <#if e.paystatus??&&e.paystatus=="y">
                <#if e.status??&&e.status=="init">
                    ...
                    <input type="button" id="addPayBtn" onclick="return addPayFunc(this);" value="添加支付记录"
                           class="btn btn-warning"/>
                    <input type="button" id="updatePayMoneryBtn" onclick="return updatePayMoneryFunc(this);"
                           value="修改订单总金额" class="btn btn-warning"/>
                ...
                <#elseif e.status??&&e.status=="pass">
                    ...
                    <input type="button" id="updatePayMoneryBtn" onclick="return updatePayMoneryFunc(this);"
                           value="修改订单总金额" class="btn btn-warning" disabled="disabled"/>
                    <a href="updateOrderStatus?id=${e.id!""}&status=cancel" onclick="return onSubmit(this);"
                       value="取消订单" class="btn btn-danger"><@i18n.message key="order_status_cancel"/></a>
                ...
                <#elseif e.status??&&e.status=="cancel">
                    ...
                    <a href="updateOrderStatus?id=${e.id!""}&status=sign" onclick="return onSubmit(this);"
                       value="已签收" class="btn" disabled="disabled"><@i18n.message key="order_status_sign"/></a>
                    <a href="updateOrderStatus?id=${e.id!""}&status=file" onclick="return onSubmit(this);"
                       value="已归档" class="btn btn-primary"
                       disabled="disabled"><@i18n.message key="order_status_file"/></a>
            ...
            <#elseif e.status??&&e.status=="cancel">
                ...
                <input type="button" id="addPayBtn" onclick="return addPayFunc(this);" value="添加支付记录"
                       class="btn btn-warning" disabled="disabled"/>
                <input type="button" id="updatePayMoneryBtn" onclick="return updatePayMoneryFunc(this);" value="修改订单总金额"
                       class="btn btn-warning" disabled="disabled"/>
                <a href="updateOrderStatus?id=${e.id!""}&status=cancel" onclick="return onSubmit(this);"
                   disabled="disabled"
                   value="取消订单" class="btn btn-danger"><@i18n.message key="order_status_cancel"/></a>
        ...
        <div id="addPayDiv" style="display: none;">
            <table class="table">
                ...
                <tr>
                    <td colspan="2">
                        <button type="button" method="insertOrderpay" onclick="selectList(this);"
        ...
        <div id="updatePayMoneryDiv" style="display: none;">
            <table class="table">
                ...
                <tr>
                    <td>备注</td>
                    <td>
                        <div class="controls"><input name="updatePayMoneryRemark" id="updatePayMoneryRemark"
                                                     placeholder="修改订单金额备注"></div>
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
        ...
        <div id="tabs">
            <ul>
                ...
                <li><a href="#tabs-2">订单明细<#if e.lowStocks??&&e.lowStocks=="y"><font color="red">【缺货】</font></#if></a>
                </li>
                ...
                <div class="alert alert-info" style="margin-bottom: 2px;">
                    ...
                    <#if e.amountExchangeScore gt 0>
                        <span class="badge badge-default"
                              style="margin-left:20px;">消耗掉会员${e.amountExchangeScore!""}个积分点</span>
                    </#if>
                    &nbsp;&nbsp;&nbsp;&nbsp;
                    <#if e.hasGift>
                        ...
                        <td style="width: 200px;">
                            ...
                            <#if e.status??&&e.status=="init">
                                <a class="btn btn-primary"
                                   href="${basepath}/manage/order/selectOrdership?orderid=${e.id!""}">修改收货人配送信息</a>
                            <#else>
                                <a class="btn btn-primary"
                                   href="${basepath}/manage/order/selectOrdership?orderid=${e.id!""}"
                                   disabled="disabled" onclick="javascript:return false;">修改收货人配送信息</a>
                            </#if>
                        </td>
                        ...
                        <tr>
                            ...
                            <td nowrap="nowrap">
                                <a target="_blank" style="text-decoration: underline;"
                                   href="${basepath}/manage/product/toEdit?id=${item.productID!""}">
                                    ${item.productID!""}
                                </a>
                                <#if item.lowStocks?? && item.lowStocks=="y"><font color="red">【缺货】</font></#if>
                            </td>
                            <td><a target="_blank" style="text-decoration: underline;"
                                   href="${systemSetting().www}/product/${item.productID!""}.html">${item.productName!""}</a>
                                <br>
                                <#if item.giftID??>
                                    <a target="_blank" style="text-decoration: underline;"
                                       href="${basepath}/manage/gift/show?id=${item.giftID!""}">
                                        【查看赠品】
                                    </a>
                                </#if>
                            </td>
                        ...
                        <tr>
                            ...
                            <td nowrap="nowrap">&nbsp;
                                <#if item.accountType??&&item.accountType=="w">
                                    <a target="_blank"
                                       href="${basepath}/manage/account/show?account=${item.account!""}">${item.account!""}</a>
                                <#elseif item.accountType??&&item.accountType=="m">
                                    <a target="_blank"
                                       href="${basepath}/manage/user/show?account=${item.account!""}">${item.account!""}</a>
