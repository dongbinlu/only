    ...
    <div id="tabs">
        ...
        <div id="tabs-1">
            ...
            <table class="table table-bordered">
                <tr>
                    <td>未付款订单数：<a class="font-focus"
                                  href="${basepath}/manage/order/selectList?init=y&paystatus=n&status=init">${systemManager().ordersReport.getNotPayCount()}</a>
                    </td>
                    <td>已付款，但未审核订单数：<a class="font-focus" style="color: #f50;"
                                       href="${basepath}/manage/order/selectList?init=y&paystatus=y&status=init">${systemManager().ordersReport.getPayButNotPassCount()}</a>
                    </td>
                </tr>
                <tr>
                    <td>已取消订单数：<a class="font-focus"
                                  href="${basepath}/manage/order/selectList?init=y&status=cancel">${systemManager().ordersReport.getCancelCount() }</a>
                    </td>
                    <td>等待发货订单数：<a class="font-focus" style="color: #f50;"
                                   href="${basepath}/manage/order/selectList?init=y&paystatus=y&status=pass">${systemManager().ordersReport.getWaitSendGoodsCount()}</a>
                    </td>
                </tr>
            </table>
            ...
            <table class="table table-bordered">
                <tr>
                    <td>【卖家需立即处理】退款协议等待卖家确认中：
                        <a class="font-focus"
                           href="${basepath}/manage/order/selectList?init=y&refundStatus=WAIT_SELLER_AGREE">${systemManager().ordersReport.getWAIT_SELLER_AGREE() }</a>
                    </td>
                </tr>
                <tr>
                    <td>【卖家需立即处理】等待卖家收货：
                        <a class="font-focus"
                           href="${basepath}/manage/order/selectList?init=y&refundStatus=WAIT_SELLER_CONFIRM_GOODS">${systemManager().ordersReport.getWAIT_SELLER_CONFIRM_GOODS() }</a>
                    </td>
                </tr>
                ...
                <tr>
                    <td>【等待买家处理完】卖家不同意协议，等待买家修改：
                        <a class="font-focus"
                           href="${basepath}/manage/order/selectList?init=y&refundStatus=SELLER_REFUSE_BUYER">${systemManager().ordersReport.getSELLER_REFUSE_BUYER() }</a>
                    </td>
                </tr>
                <tr>
                    <td>【等待买家处理完】退款协议达成，等待买家退货：
                        <a class="font-focus"
                           href="${basepath}/manage/order/selectList?init=y&refundStatus=WAIT_BUYER_RETURN_GOODS">${systemManager().ordersReport.getWAIT_BUYER_RETURN_GOODS() }</a>
                    </td>
                </tr>
            </table>
            ...
            <table class="table table-bordered">
                <tr>
                    <td>缺货商品数：<a class="font-focus"
                                 href="${basepath}/manage/product/selectList?init=y&selectOutOfStockProduct=true">${systemManager().ordersReport.getOutOfStockProductCount()}</a>
                    </td>
                    <td>未回复的吐槽评论数：<a class="font-focus"
                                     href="${basepath}/manage/comment/selectList?init=y&selectCommentFromIndex=true">${systemManager().ordersReport.getNotReplyCommentCount()}</a>
                    </td>
                </tr>
            </table>
        </div>
        <div id="tabs-2">
            <table class="table table-condensed">
                ...
                <tr>
                    ...
                    <td style="text-align: left;">
                        <input type="checkbox" disabled="disabled" checked="${systemSetting().getIsopen() }"
                               value="${systemSetting().getIsopen() }"/>
                    </td>
                </tr>
                <tr>
