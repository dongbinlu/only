        ...
        <table class="table table-bordered">
            ...
            <tr>
                ...
                <td style="text-align: left;">
                    <input type="text" value="${e.name!""}" name="name" id="name"
                           data-rule="活动名称:required;name;length[1~45];"/>
                </td>
            </tr>
            <tr>
                ...
                <td style="text-align: left;" class="dfsfdsfsf" id="checkboxArr">
                    ...
                    <input type="hidden" id="__multiselect_form_e_accountRange" name="__multiselect_e.accountRange"
                           value="${e.accountRange!""}">
                ...
                <td style="text-align: left;">
                    ...
                    <select id="status" name="status" class="input-small" data-rule="状态:required;status;">
                        <#list map?keys as key>
                            <option value="${key}"
                                    <#if e.status?? && e.status==key>selected="selected" </#if>>${map[key]}</option>
                        </#list>
                    </select>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td style="text-align: left;">
                    ...
                    <select id="activityType" name="activityType" class="input-small"
                        ...
                        <#list map?keys as key>
                            <option value="${key}"
                                    <#if e.activityType?? && e.activityType==key>selected="selected" </#if>>${map[key]}</option>
                        </#list>
                    </select>
                    <p>
                    <div id="discountDiv" style="display: none;">
                        <p>优惠方式：
                            ...
                            <select id="discountType" name="discountType" class="input-small"
                                ...
                                <#list map?keys as key>
                                    <option value="${key}"
                                            <#if e.discountType?? && e.discountType==key>selected="selected" </#if>>${map[key]}</option>
                                </#list>
                            </select>
                        <p>折扣/减价：<input type="text" value="${e.discount!""}" id="discount" name="discount"
                                        class="input-small"/></p>
                    </div>

                    <div id="exchangeScoreDiv" style="display: none;">
                        兑换积分：<input type="text" value="${e.exchangeScore!""}" id="exchangeScore" name="exchangeScore"
                                    class="input-small"/>
                    </div>

                    <div id="minGroupCountDiv" style="display: none;">
                        <p>最低团购人数：<input type="text" value="${e.minGroupCount!""}" id="minGroupCount"
                                         name="minGroupCount" class="input-small"/></p>
                        <p>团购价：<input type="text" value="${e.tuanPrice!""}" id="tuanPrice" name="tuanPrice"
                                      class="input-small"/></p>
                    </div>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td style="text-align: left;">
                    <input type="text" value="${e.maxSellCount!""}" id="maxSellCount" name="maxSellCount"
                           class="input-small" data-rule="最大购买数量:required;integer;maxSellCount;"/>
                    <br>
                    (0：表示不限制)
                </td>
            </tr>
