package com.safecode.es.config;
...
public class ElasticSearchManager implements InitializingBean {
    ...
    public void init() {
        settings = Settings.builder().put("cluster.name", "my-es").build();
        try {
            client = new PreBuiltTransportClient(settings).addTransportAddress(
                    new TransportAddress(InetAddress.getByName("192.168.50.28"), Integer.parseInt("19300")));// 你们要xie'wan
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static void main(String[] args) {
        ...
        SearchResponse response = elasticSearchManager.client.prepareSearch("bbg_goods").setTypes("item_loc")
