 ...
 */
package com.safecode.cyberzone.auth.factory;
...
import com.safecode.cyberzone.auth.controller.RsaController;
import com.safecode.cyberzone.auth.exception.AuthException;
import com.safecode.cyberzone.auth.exception.SsoOAuth2Exception;
import com.safecode.cyberzone.auth.utils.RsaUtil;
 ...
 */
public class SSOOAuth2RequestFactory implements OAuth2RequestFactory {
    ...
    public SSOOAuth2RequestFactory(ClientDetailsService clientDetailsService) {
     ...
     * No scopes will be granted to a user unless they are permitted as a
     * granted authority to that user.
     *
    ...
    public TokenRequest createTokenRequest(Map<String, String> requestParameters, ClientDetails authenticatedClient) {
        try {
            String decrypt = RsaUtil.decrypt(requestParameters.get("password"),
                    RsaUtil.getPrivateKey(RsaController.class.getResourceAsStream("/rsa/rsa_1024_priv.pem")));
            requestParameters.put("password", decrypt);
        } catch (Exception e) {
            log.error("密码处理异常 ", e);
        ...
        if (clientId == null) {
            // if the clientId wasn't passed in in the map, we add pull it from
    ...
    private Set<String> extractScopes(Map<String, String> requestParameters, String clientId) {
        ...
        if ((scopes == null || scopes.isEmpty())) {
            // If no scopes are specified in the incoming data, use the default
            // values registered with the client
            // (the spec allows us to choose between this option and rejecting
            // the request completely, so we'll take the
